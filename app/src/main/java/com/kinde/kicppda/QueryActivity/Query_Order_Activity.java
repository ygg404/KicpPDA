...
public class Query_Order_Activity extends DecodeBaseActivity implements View.OnClickListener,OnEngineStatus{
	...
	private OrderEntity orderEntity;       //选中的主单
    private OrderBillingEntity billingEntity;  //选中的明细
    ...
    private void queryPopInit(View v){
        ...
        //创建SimpleAdapter适配器将数据绑定到item显示控件上
        digAdapter = new SimpleAdapter(Query_Order_Activity.this, ProductInfo, R.layout.query_list_items,
                new String[]{"qcode", "qname" }, new int[]{R.id.qcode, R.id.qname });

        String keyValue = tbProduct.getText().toString();
        ForeignCollection<OrderBillingEntity> billingEntities = orderEntity.bNotes;
        Iterator<OrderBillingEntity> billIterator = billingEntities.iterator();
    ...
    public void loadBarcodeList(){
        ScanInfoList.clear();
        ForeignCollection<OrderScanEntity> scanEntities = billingEntity.sNotes;
