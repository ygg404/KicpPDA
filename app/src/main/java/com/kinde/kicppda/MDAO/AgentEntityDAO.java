...
public class AgentEntityDAO {
    private Context context;
    // ORMLite提供的DAO类对象，第一个泛型是要操作的数据表映射成的实体类；第二个泛型是这个实体类中ID的数据类型
    private Dao<AgentEntity, String> dao;

    public AgentEntityDAO(Context context){
        this.context = context;
        try{
            this.dao = DBHelper.getInstance(context).getDao(AgentEntity.class);
    ...
    public void insert(AgentEntity data) {
        try {
    ...
    public void delete(AgentEntity data) {
        try {
            dao.delete(data);
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    // 查询所有记录
    public List<AgentEntity> queryForAll(){
    ...
    // 通过条件查询明细单
    public List<AgentEntity> queryByEq(String eq , String value) {
        List<AgentEntity> entities = new ArrayList<>();
        try {
