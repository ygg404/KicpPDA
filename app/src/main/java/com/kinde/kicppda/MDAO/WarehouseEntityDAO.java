...
public class WarehouseEntityDAO {
    private Context context;
    // ORMLite提供的DAO类对象，第一个泛型是要操作的数据表映射成的实体类；第二个泛型是这个实体类中ID的数据类型
    private Dao<WarehouseEntity, String> dao;

    public WarehouseEntityDAO(Context context){
        this.context = context;
        try{
            this.dao = DBHelper.getInstance(context).getDao(WarehouseEntity.class);
    ...
    public void insert(WarehouseEntity data) {
        try {
    ...
    public void delete(WarehouseEntity data) {
        try {
            dao.delete(data);
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    // 查询所有记录
    public List<WarehouseEntity> queryForAll(){
    ...
    // 通过条件查询明细单
    public List<WarehouseEntity> queryByEq(String eq , String value) {
        List<WarehouseEntity> entities = new ArrayList<>();
        try {
